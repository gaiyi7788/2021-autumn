...
if __name__=="__main__":
    ...
    optimizer = torch.optim.SGD(mcnn.parameters(), lr=1e-6,
                                momentum=0.95)
    root = os.getcwd()
    img_root = os.path.join(root,'data_preparation','ShanghaiTech/part_A/train_data','images')
    gt_dmap_root = os.path.join(root,'data_preparation','ShanghaiTech/part_A/train_data','ground_truth')
    dataset=CrowdDataset(img_root,gt_dmap_root,4)
    dataloader=torch.utils.data.DataLoader(dataset,batch_size=1,shuffle=True)

    test_img_root = os.path.join(root,'data_preparation','ShanghaiTech/part_A/test_data','images')
    test_gt_dmap_root = os.path.join(root,'data_preparation','ShanghaiTech/part_A/test_data','ground_truth')
